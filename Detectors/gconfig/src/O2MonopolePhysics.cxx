...
/// physics lists. As a consequence a simulated monopole deposits no dE/dx and
/// leaves no hits/clusters.
///
/// This file adds - only when explicitly requested via G4Params.monopole - the
/// magnetic-monopole ionisation process G4mplIonisation (Ahlen stopping power,
/// Rev. Mod. Phys. 52 (1980) 121) to those particles. The implementation mirrors
/// Geant4's own examples/extended/exoticphysics/monopole example (and the
/// equivalent geant4_vmc "monopole" physics builder), the difference being that
/// here the process is attached to the pre-existing O2 monopole particles rather
/// than to a freshly created G4Monopole (whose PDG code, 60000000, would not
/// match the ones produced by the O2 generators).
///
/// G4mplIonisation and G4mplIonisationWithDeltaModel are shipped in the standard
/// Geant4 libraries (libG4processes), so no third-party source needs to be
/// vendored.
 ...
 public:
  ...
  void ConstructProcess() override
  {
    ...
    for (int pdg : gMonopolePDGs) {
      ...
      // (hIoni) to the monopole. For a magnetic monopole this is wrong physics
      // (and, since it is built in a different physics-list pass than mplIoni,
      // it makes G4LossTableManager crash while merging the two dE/dx tables).
      // Remove any pre-existing energy-loss process so that mplIoni is the
      // monopole's single, correct ionisation process. This mirrors Geant4's
      // exoticphysics/monopole example, where the monopole's only ionisation is
      // mplIoni.
      {
